...
- **工作目录怎么来的**：四层兜底，顺序照官方 `dsh-api-workspace-files` 的 `workspaceFileScope.resolve`：
  ① 会话事件里记的；② `ctx.sessions.get(id)`（这个宿主进程里活着的会话）；
  ③ `ctx.get('sessionPersistence').stat(id).header.cwd`（**落盘但还没加载**的会话 —— 重启后立刻打开 tab 就是这种）；
  ④ `ctx.sandboxPolicy.workspaceRoot`（进程默认工作区）。
  路径只从这四处来，不接受客户端传。早先只做了 ①②，所以**重启后立刻打开 tab 会报「拿不到这个会话的工作目录」**。
